...
export const CALL_PERMISSIONS = Platform.select({
  android: parseFloat(deviceInfoModule.getSystemVersion()) > 11 ? [PERMISSIONS.ANDROID.CAMERA, PERMISSIONS.ANDROID.RECORD_AUDIO, PERMISSIONS.ANDROID.BLUETOOTH_CONNECT] : [PERMISSIONS.ANDROID.CAMERA, PERMISSIONS.ANDROID.RECORD_AUDIO],
...
export const usePermissions = (perms: Permission[]) => {
  const [state, setState] = useState<'pending' | 'granted' | 'rejected'>(
    'pending',
  );
  useEffect(() => {
