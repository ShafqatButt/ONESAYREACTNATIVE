    ...
    if (err.response) {
      switch (err.response.status) {
        case 400:
          ...
          const {messages} = err?.response?.data?.ui;

          if (messages?.length > 0) {
            const errorMessage = Array.prototype.concat(
              ...messages?.map(
                (data, index) =>
                  data.text + (index === messages.length - 1 ? '' : '\n'),
              ),
            )[0];
            console.debug('Form validation failed:', errorMessage);
            showMessage({
